...
export const matchPlan: MatchPlanItem[] = (() => {
  const segments = [4, 5, 6, 4, 5, 6];
  const list: MatchPlanItem[] = [];
  let n = 1;
  segments.forEach((size, idx) => {
    for (let i = 1; i <= size; i += 1) {
      list.push({ matchNumber: n++, block: idx + 1, roundType: i === size ? "golden" : "normal" });
    }
  });
  return list;
})();
