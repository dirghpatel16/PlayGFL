...
export async function GET(req: NextRequest) {
  ...
  const profiles = await supabaseAdminTable<any[]>("player_profiles?select=user_id,bgmi_name,bgmi_id,preferred_roles").catch(() => []);

  const withUser = rows.map((r) => {
    const u = users.find((x) => x.id === r.user_id);
    const p = profiles.find((x) => x.user_id === r.user_id);
    return { ...r, username: u?.username ?? "Unknown", email: u?.email ?? "", bgmi_name: p?.bgmi_name, bgmi_id: p?.bgmi_id, preferred_roles: p?.preferred_roles ?? [] };
  });

  const filtered = search ? withUser.filter((r) => `${r.username} ${r.payer_name ?? ""} ${r.utr ?? ""}`.toLowerCase().includes(search)) : withUser;
